...
from double_pendulum import P,Q,R,controller,dynamics
...
@lru_cache(maxsize=1)
def tracker():
    ref=np.load(ROOT/'results/double/swingup_reference.npz')
    T=float(ref['times'][-1])
    ...
    Ksteady,S=controller();gains=np.zeros((len(times),6));gains[-1]=Ksteady
    eps=1e-5
    for i in range(len(times)-2,-1,-1):
        s=states[i];u=forces[i]
        A=np.column_stack([(dynamics(s+e*eps,u)-dynamics(s-e*eps,u))/(2*eps) for e in np.eye(6)])
        B=((dynamics(s,u+eps)-dynamics(s,u-eps))/(2*eps))[:,None]
        Ad=np.eye(6)+A*dt+.5*A@A*dt**2;Bd=B*dt+.5*A@B*dt**2
        K=np.linalg.solve(np.array([[R*dt]])+Bd.T@S@Bd,Bd.T@S@Ad)
        S=np.diag(Q)*dt+Ad.T@S@Ad-Ad.T@S@Bd@K;S=(S+S.T)/2
        gains[i]=K[0]
    def command(t,s):
        if t>=T:return float(np.clip(-Ksteady@s,-P.force_limit,P.force_limit))
        refstate=spline(t);gain=np.array([np.interp(t,times,gains[:,j]) for j in range(6)])
...
def simulate(controlled=True,dt=.0025,duration=12.):
    command,T=tracker();s=np.array([0.,0.,np.pi,0.,np.pi,0.]);rows=[];status='completed'
    ...
    bad=np.flatnonzero(~within|(data[:,0]<T));first=int(bad[-1]+1) if len(bad) else 0
    settling=float(data[first,0]) if status=='completed' and first<len(data) and data[-1,0]-data[first,0]>=1 else None
    return data,dict(status=status,end_s=float(data[-1,0]),settling_s=settling,peak_force_N=float(abs(data[:,7]).max()),effort_N2s=float(np.sum(np.diff(data[:,0])*(data[:-1,7]**2+data[1:,7]**2)/2)),handoff_s=T)
...
def main():
    for key,enabled in [('lqr',True),('open',False)]:
        ...
        np.savetxt(ROOT/f'results/double/swingup_{key}.csv',data,delimiter=',',header='time_s,x_m,v_m_s,theta1_rad,omega1_rad_s,theta2_rad,omega2_rad_s,force_N,push_N',comments='')
