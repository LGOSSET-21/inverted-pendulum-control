...
from triple_pendulum import Q,R,controller,dynamics,metrics,batch_dynamics
from pendulum import rk4
ROOT=Path(__file__).resolve().parents[1]

@lru_cache(maxsize=1)
def tracker():
    ref=np.load(ROOT/'results/triple/swingup_reference.npz')
    T=float(ref['times'][-1])
...
def main():
    for key,enabled in [('lqr',True),('open',False)]:
        ...
        np.savetxt(ROOT/f'results/triple/swingup_{key}.csv',data,delimiter=',',header='time_s,x_m,v_m_s,theta1_rad,omega1_rad_s,theta2_rad,omega2_rad_s,theta3_rad,omega3_rad_s,force_N,push_N',comments='')
