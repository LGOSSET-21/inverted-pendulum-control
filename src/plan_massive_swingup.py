...
def plan():
    ...
    checkpoint=ROOT/'results/massive_rods/swingup_attempt.npz'
    if not checkpoint.exists():checkpoint=ROOT/'results/triple/swingup_reference.npz'
    if checkpoint.exists():
        ...
        if previous['states'].shape==(N,nx) and np.allclose(previous['times'],times):
            ...
            print('Loading initial guess (re-optimised with massive-rod dynamics)',flush=True)
    def residual(flat):
